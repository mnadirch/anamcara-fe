...
interface ChatInputProps {
  onSendMessage: (message: string) => Promise<void>; // Updated to async
}
...
const ChatInput: React.FC<ChatInputProps> = ({ onSendMessage }) => {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(false); // Move loading inside this component
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      handleSubmit(e as unknown as FormEvent); // Cast to satisfy types
    }
  };
  ...
  return (
    <div className="flex bg-opacity-80 z-20 relative mb-4">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col w-full max-w-[670px]">
        <div
          className={`relative flex items-end rounded-lg overflow-hidden bg-gray-900 border
            animate-rainbow hover:animate-rainbow-reverse focus-within:animate-rainbow-reverse transition-all duration-300`}
        >
          <button
            type="button"
          ...
          <button
            type="submit"
            className={`absolute bottom-2 right-2 p-2 rounded-full border transition-all duration-300 ${
