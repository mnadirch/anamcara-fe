...
import { FiMessageSquare, FiPlus, FiSettings, FiLogOut, FiAlertCircle, FiSearch } from 'react-icons/fi';
...
const Sidebar: React.FC<SidebarProps> = ({ onToggleNotifications, isNotificationOff, isOpen, onClose }) => {
  ...
  const handleConfirmLogout = async () => {
    try {
      await AuthService.signOut(); // ← using your AuthService method
              ...
              {conversations.map((conversation) => (
                <li key={conversation.id} className="mb-1">
                  <button
                    ...
                    className={`p-2 w-full rounded-md flex items-center transition-colors  ${
