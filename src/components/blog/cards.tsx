...
const Cards: React.FC<CardsProps> = ({
  ...
  return (
    <div className="w-fit lg:h-full lg:pt-20 z-20 fixed bottom-0 left-0 flex flex-col lg:relative md:py-6 py-3">
              ...
              <div className="w-full flex h-full space-x-6 px-8 py-2">
                {blogs?.map((card) => {
                  ...
                  return (
                    <div
                      key={card.id}
                      className={`bg-[#505050] bg-opacity-70 max-h-[240px] font-calibri flex-grow h-full text-white p-4 sm:p-6 rounded-xl shadow-lg cursor-pointer max-w-[300px] snap-center flex flex-col justify-between shrink-0 transition-all duration-300 border-2 ${isActive === card.blogId ? 'opacity-100 border-[#ADFF00]' : ' scale-95 opacity-70 border-transparent'}`}
                    ...
                    >
                      ...
                      <h3 className="text-sm h-9 font-bold transition-transform duration-700 group-hover:translate-y-[-5px]">
